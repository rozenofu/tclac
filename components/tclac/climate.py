    ...
    if CONF_VISUAL in config:
        ...
        if CONF_MIN_TEMPERATURE in visual_config:
            ...
            if min_temp < TCLAC_MIN_TEMPERATURE:
                raise cv.Invalid(f"min_temperature {min_temp} is below AC minimum {TCLAC_MIN_TEMPERATURE}")
        else:
        ...
        if CONF_MAX_TEMPERATURE in visual_config:
            ...
            if max_temp > TCLAC_MAX_TEMPERATURE:
                raise cv.Invalid(f"max_temperature {max_temp} exceeds AC maximum {TCLAC_MAX_TEMPERATURE}")
        else:
            config[CONF_VISUAL][CONF_MAX_TEMPERATURE] = TCLAC_MAX_TEMPERATURE
        if CONF_TEMPERATURE_STEP not in visual_config:
...
CONFIG_SCHEMA = cv.All(
    ...
    .extend({
        ...
        cv.Optional(CONF_SUPPORTED_FAN_MODES, default=["AUTO", "QUIET", "LOW", "MIDDLE", "MEDIUM", "HIGH", "FOCUS", "DIFFUSE"]):
